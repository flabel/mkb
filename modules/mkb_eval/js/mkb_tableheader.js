...
//	    	hdata[i-2][hcells.length-1] = hdata[i-2][hcells.length-1] + cell;
	    	// Requested
	    	if(i==2){
	    		requested += cell;
	    	}
...
//	    	hcells[hcells.length-1].innerHTML = deficit_total;
	    	hcells[hcells.length-1].innerHTML = deficit;
	    	if(deficit_total<0){
...
//	    hcells[hcells.length].innerHTML = hdata[i-2][hdata[i-2].length]; 
	}
	
	
	
	table = document.getElementById('application_selection_table');
