...
foreach ($rows as $row_count => $row) {
  if (!$row['field_event_registration'] || $row['field_event_registration'] == 'No' || !user_is_logged_in()) {
    $rows[$row_count]['nothing'] = '';
	if (!user_is_logged_in()) unset($rows[$row_count]['nothing']);
  }
  unset($rows[$row_count]['field_event_registration']);
}
unset($header['field_event_registration']);
if (!user_is_logged_in()) unset($header['nothing']);
...
foreach ($rows as $row_count => $row) {
  $time_ago = $row['field_event_days_1'];
